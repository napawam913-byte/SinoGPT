...
def sample_next_token(logits: Tensor, temperature: float, top_k: int) -> int:
    """按温度和可选 top-k 从单一位置 logits 中采样一个 token ID。"""
    if logits.ndim != 1:
...
def generate_assistant_ids(
    ...
    *,
    ...
    """从 assistant 标记后生成 token，遇 EOS 或达到上限时停止。"""
    if not prompt_ids:
    ...
    try:
        for _ in range(max_new_tokens):
            ...
            next_id = sample_next_token(logits[0, -1], temperature, top_k)
